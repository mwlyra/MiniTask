...
Version 1.1 measured executable size: **173,098,554 bytes (165.1 MiB)**. This is much larger than TinyTask because it includes the managed runtime and Windows desktop framework. ZIP compression reduces download size; the precise final archive size and SHA-256 values are recorded in the accompanying `release-metrics-v1.1.json` and `SHA256SUMS-v1.1.txt`.
